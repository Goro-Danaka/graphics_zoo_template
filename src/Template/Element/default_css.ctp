<?php
/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
?>
<!-- START GLOBAL CSS -->
<?= $this->Html->css('/theme/assets/global/plugins/bootstrap/dist/css/bootstrap.min.css'); ?>
<?= $this->Html->css('/theme/assets/icons_fonts/elegant_font/elegant.min.css'); ?>
<?= $this->Html->css('/theme/assets/layouts/layout-left-top-menu/css/color/light/color-default.css'); ?>
<?= $this->Html->css('/theme/assets/global/plugins/switchery/dist/switchery.min.css'); ?>
<?= $this->Html->css('/theme/assets/global/plugins/perfect-scrollbar/css/perfect-scrollbar.min.css'); ?>
<?= $this->Html->css('/theme/assets/global/plugins/dropify/dist/css/dropify.min.css'); ?>
<?= $this->Html->css('/theme/assets/global/plugins/dropify/dist/css/dropify.min.css'); ?>
<?= $this->Html->css('/theme/assets/global/plugins/font-awesome-4.7.0/css/font-awesome.min.css'); ?>
<?= $this->Html->css('/theme/assets/global/plugins/bootstrap-wysiwyg/css/code_view.css'); ?>
<?= $this->Html->css('/theme/assets/global/plugins/bootstrap-wysiwyg/css/image_manager.css'); ?>
<?= $this->Html->css('/theme/assets/global/plugins/bootstrap-wysiwyg/css/image.css'); ?>
<?= $this->Html->css('/theme/assets/global/plugins/bootstrap-wysiwyg/css/table.css'); ?>
<?= $this->Html->css('/theme/assets/global/plugins/bootstrap-wysiwyg/css/video.css'); ?> 
<!-- END GLOBAL CSS -->

<?= $this->Html->css('/theme/assets/global/plugins/datatables/media/css/dataTables.bootstrap4.min.css'); ?>
<?= $this->Html->css('/theme/assets/global/plugins/datatables-responsive/css/responsive.bootstrap4.min.css'); ?>
<?= $this->Html->css('/theme/assets/global/plugins/datatables-scroller/css/scroller.bootstrap4.min.css'); ?>
<!-- Include Editor style. -->


<!-- START PAGE PLUG-IN CSS -->
<?php // $this->Html->css('/assets/global/plugins/font-awesome/css/font-awesome.min.css'); ?>
<?php // $this->Html->css('/assets/global/plugins/weather-icons/css/weather-icons.min.css'); ?>
<?php // $this->Html->css('/assets/global/plugins/weather-icons/css/weather-icons-wind.min.css'); ?>
<?php // $this->Html->css('/assets/global/plugins/rickshaw/rickshaw.min.css'); ?>
<!-- END PAGE PLUG-IN CSS -->

<!-- START TEMPLATE GLOBAL CSS -->
<?= $this->Html->css('/theme/assets/layouts/layout-left-menu/css/color/light/color-spice.min.css'); ?>
<?= $this->Html->css('/theme/assets/layouts/layout-left-menu/css/color/light/color-dodger-blue.min.css'); ?>

<!-- START TEMPLATE GLOBAL CSS -->
<?= $this->Html->css('/theme/assets/global/css/components.min.css'); ?>
<?= $this->Html->css('/theme/assets/layouts/layout-left-menu/css/layout.min.css'); ?>

<!-- START CUSTOME CSS -->
<?= $this->Html->css('/theme/assets/global/css/style-datatable.css'); ?>
<?= $this->Html->css('/theme/assets/global/css/style-datatable.css'); ?>
<?= $this->Html->css('custom.css'); ?>

<!-- plugins css -->
<?= $this->Html->css('/assets/bower_components/bootstrap/dist/css/bootstrap.css'); ?>
<?= $this->Html->css('/assets/bower_components/PACE/themes/blue/pace-theme-minimal.css'); ?>
<?= $this->Html->css('/assets/bower_components/perfect-scrollbar/css/perfect-scrollbar.min.css'); ?>
    

<!-- page plugins css -->
<?= $this->Html->css('/assets/bower_components/bower-jvectormap/jquery-jvectormap-1.2.2.css'); ?>
<?= $this->Html->css('/assets/bower_components/nvd3/build/nv.d3.min.css'); ?>
<?= $this->Html->css('/assets/bower_components/datatables/media/css/jquery.dataTables.css'); ?>
<?= $this->Html->css('/assets/bower_components/photoswipe/dist/photoswipe.css'); ?>
<?= $this->Html->css('/assets/bower_components/photoswipe/dist/default-skin/default-skin.css'); ?>

<!-- core css -->
<?= $this->Html->css('/assets/assets/css/ei-icon.css'); ?>
<?= $this->Html->css('/assets/assets/css/themify-icons.css'); ?>
<?= $this->Html->css('/assets/assets/css/font-awesome.min.css'); ?>
<?= $this->Html->css('/assets/assets/css/animate.min.css'); ?>
<?= $this->Html->css('/assets/assets/css/app.css'); ?>


    